...
PROJECT_NAME="CLion Professional"
ARTIFACT_NAME="clion-pro"
...
print_info() {
  ...
  printf ">>    SOURCE                   = https://download-cdn.jetbrains.com/cpp/CLion-*.tar.gz\n"
...
mv                 %{buildroot}%{libdir}/bin/clion64.vmoptions  %{buildroot}%{_sysconfdir}/jetbrains/
mkdir -p           %{buildroot}%{_datadir}/icons/jetbrains/
mkdir -p           %{buildroot}%{_datadir}/icons/hicolor/128x128/apps/
mv                 %{buildroot}%{libdir}/bin/clion.png  %{buildroot}%{_datadir}/icons/hicolor/128x128/apps/%{name}.png
mkdir -p           %{buildroot}%{_datadir}/icons/hicolor/scalable/apps/
mv                 %{buildroot}%{libdir}/bin/clion.svg   %{buildroot}%{_datadir}/icons/hicolor/scalable/apps/%{name}.svg
...
%config(noreplace) %{_sysconfdir}/jetbrains/clion64.vmoptions
...
gen_patch_files() {
  ...
  # create .patch files
  cat << EOF > $PATCH_PATH/bin_clion.sh.patch
42a43,44
...
patch_unpacked() {
  printf ">>    (3.1)                    » patching bin/clion.sh\n"
  patch  $REPACK_PATH/bin/clion.sh < $PATCH_PATH/bin_clion.sh.patch
  printf ">>    (3.2)                    » patching bin/"$ARTIFACT_NAME"\n"
  cat << EOF > $REPACK_PATH/bin/$ARTIFACT_NAME
#!/usr/bin/bash

#  set environment overrides
# export CLION_JDK=/usr/lib/jvm/java-21/
export IDE_HOME=/usr/lib64/jetbrains/$ARTIFACT_NAME-$VERSION
export VM_OPTIONS_FILE=/etc/jetbrains/clion64.vmoptions

#  launch clion
exec /usr/lib64/jetbrains/$ARTIFACT_NAME-$VERSION/bin/clion.sh \$@
EOF
  printf ">>    (3.3)                    » patching com.jetbrains."$ARTIFACT_NAME".desktop\n"
  cat << EOF > $REPACK_PATH"/com.jetbrains."$ARTIFACT_NAME".desktop"
[Desktop Entry]
Name=Clion Pro Edition
GenericName=The intelligent C & C++ IDE
...
StartupNotify=true
MimeType=application/x-c-code;text/x-c;
EOF
