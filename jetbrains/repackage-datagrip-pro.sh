...
PROJECT_NAME="DataGrip Professional"
ARTIFACT_NAME="datagrip-pro"
...
print_info() {
  ...
  printf ">>    SOURCE                   = https://download-cdn.jetbrains.com/datagrip/datagrip-*.tar.gz\n"
...
mv                 %{buildroot}%{libdir}/bin/datagrip64.vmoptions  %{buildroot}%{_sysconfdir}/jetbrains/
mkdir -p           %{buildroot}%{_datadir}/icons/jetbrains/
mkdir -p           %{buildroot}%{_datadir}/icons/hicolor/128x128/apps/
mv                 %{buildroot}%{libdir}/bin/datagrip.png  %{buildroot}%{_datadir}/icons/hicolor/128x128/apps/%{name}.png
mkdir -p           %{buildroot}%{_datadir}/icons/hicolor/scalable/apps/
mv                 %{buildroot}%{libdir}/bin/datagrip.svg   %{buildroot}%{_datadir}/icons/hicolor/scalable/apps/%{name}.svg
...
%config(noreplace) %{_sysconfdir}/jetbrains/datagrip64.vmoptions
...
gen_patch_files() {
  ...
  # create .patch files
  cat << EOF > $PATCH_PATH/bin_datagrip.sh.patch
42a43,44
...
patch_unpacked() {
  printf ">>    (3.1)                    » patching bin/datagrip.sh\n"
  patch  $REPACK_PATH/bin/datagrip.sh < $PATCH_PATH/bin_datagrip.sh.patch
  printf ">>    (3.2)                    » patching bin/"$ARTIFACT_NAME"\n"
  cat << EOF > $REPACK_PATH/bin/$ARTIFACT_NAME
#!/usr/bin/bash

#  set environment overrides
# export DATAGRIP_JDK=/usr/lib/jvm/java-21/
export IDE_HOME=/usr/lib64/jetbrains/$ARTIFACT_NAME-$VERSION
export VM_OPTIONS_FILE=/etc/jetbrains/datagrip64.vmoptions

#  launch datagrip
exec /usr/lib64/jetbrains/$ARTIFACT_NAME-$VERSION/bin/datagrip.sh \$@
EOF
  printf ">>    (3.3)                    » patching com.jetbrains."$ARTIFACT_NAME".desktop\n"
  cat << EOF > $REPACK_PATH"/com.jetbrains."$ARTIFACT_NAME".desktop"
[Desktop Entry]
Name=DataGrip Pro Edition
...
StartupNotify=true
MimeType=application/x-sql-code;text/x-sql;
EOF
...
URL="https://download-cdn.jetbrains.com/datagrip/datagrip-"$VERSION".tar.gz"
