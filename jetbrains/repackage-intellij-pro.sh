...
PROJECT_NAME="IntelliJ Professional"
ARTIFACT_NAME="intellij-pro"
...
print_info() {
  ...
  printf ">>    SOURCE                   = https://download-cdn.jetbrains.com/idea/ideaIU-*.tar.gz\n"
...
cp -nr             jdk-shared-indexes/                     %{buildroot}%{libdir}/
...
mv                 %{buildroot}%{libdir}/bin/idea64.vmoptions  %{buildroot}%{_sysconfdir}/jetbrains/intellij64.vmoptions
mkdir -p           %{buildroot}%{_datadir}/icons/jetbrains/
mkdir -p           %{buildroot}%{_datadir}/icons/hicolor/128x128/apps/
mv                 %{buildroot}%{libdir}/bin/idea.png  %{buildroot}%{_datadir}/icons/hicolor/128x128/apps/%{name}.png
mkdir -p           %{buildroot}%{_datadir}/icons/hicolor/scalable/apps/
mv                 %{buildroot}%{libdir}/bin/idea.svg   %{buildroot}%{_datadir}/icons/hicolor/scalable/apps/%{name}.svg
...
%config(noreplace) %{_sysconfdir}/jetbrains/intellij64.vmoptions
%{libdir}/Install-Linux-tar.txt
%{libdir}/product-info.json
%{libdir}/build.txt
%{libdir}/bin/
%{libdir}/help/
%{libdir}/jdk-shared-indexes/
%{libdir}/lib/
%{libdir}/license/
%{libdir}/modules/
%{libdir}/plugins/
...
gen_patch_files() {
  ...
  # create .patch files
  cat << EOF > $PATCH_PATH/bin_idea.sh.patch
42a43,44
...
patch_unpacked() {
  printf ">>    (3.1)                    » patching bin/idea.sh\n"
  patch  $REPACK_PATH/bin/idea.sh < $PATCH_PATH/bin_idea.sh.patch
  printf ">>    (3.2)                    » patching bin/"$ARTIFACT_NAME"\n"
  cat << EOF > $REPACK_PATH/bin/$ARTIFACT_NAME
#!/usr/bin/bash

#  set environment overrides
# export IDEA_JDK=/usr/lib/jvm/java-21/
export IDE_HOME=/usr/lib64/jetbrains/$ARTIFACT_NAME-$VERSION
export VM_OPTIONS_FILE=/etc/jetbrains/intellij64.vmoptions

#  launch intellij
exec /usr/lib64/jetbrains/$ARTIFACT_NAME-$VERSION/bin/idea.sh \$@
EOF
  printf ">>    (3.3)                    » patching com.jetbrains."$ARTIFACT_NAME".desktop\n"
  cat << EOF > $REPACK_PATH"/com.jetbrains."$ARTIFACT_NAME".desktop"
[Desktop Entry]
Name=IntelliJ Pro Edition
GenericName=The intelligent Java IDE
...
StartupNotify=true
MimeType=application/x-java-code;text/x-java;
EOF
