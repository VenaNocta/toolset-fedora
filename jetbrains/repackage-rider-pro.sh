...
PROJECT_NAME="Rider Professional"
ARTIFACT_NAME="rider-pro"
...
print_info() {
  ...
  printf ">>    SOURCE                   = https://download-cdn.jetbrains.com/rider/JetBrains.Rider-*.tar.gz\n"
...
mv                 %{buildroot}%{libdir}/bin/rider64.vmoptions  %{buildroot}%{_sysconfdir}/jetbrains/
mkdir -p           %{buildroot}%{_datadir}/icons/jetbrains/
mkdir -p           %{buildroot}%{_datadir}/icons/hicolor/128x128/apps/
mv                 %{buildroot}%{libdir}/bin/rider.png  %{buildroot}%{_datadir}/icons/hicolor/128x128/apps/%{name}.png
mkdir -p           %{buildroot}%{_datadir}/icons/hicolor/scalable/apps/
mv                 %{buildroot}%{libdir}/bin/rider.svg   %{buildroot}%{_datadir}/icons/hicolor/scalable/apps/%{name}.svg
...
%config(noreplace) %{_sysconfdir}/jetbrains/rider64.vmoptions
...
gen_patch_files() {
  ...
  # create .patch files
  cat << EOF > $PATCH_PATH/bin_rider.sh.patch
42a43,44
...
patch_unpacked() {
  printf ">>    (3.1)                    » patching bin/rider.sh\n"
  patch  $REPACK_PATH/bin/rider.sh < $PATCH_PATH/bin_rider.sh.patch
  printf ">>    (3.2)                    » patching bin/"$ARTIFACT_NAME"\n"
  cat << EOF > $REPACK_PATH/bin/$ARTIFACT_NAME
#!/usr/bin/bash

#  set environment overrides
# export RIDER_JDK=/usr/lib/jvm/java-21/
export IDE_HOME=/usr/lib64/jetbrains/$ARTIFACT_NAME-$VERSION
export VM_OPTIONS_FILE=/etc/jetbrains/rider64.vmoptions

#  launch rider
exec /usr/lib64/jetbrains/$ARTIFACT_NAME-$VERSION/bin/rider.sh \$@
EOF
  printf ">>    (3.3)                    » patching com.jetbrains."$ARTIFACT_NAME".desktop\n"
  cat << EOF > $REPACK_PATH"/com.jetbrains."$ARTIFACT_NAME".desktop"
[Desktop Entry]
Name=Rider Pro Edition
GenericName=The intelligent .NET IDE
...
StartupNotify=true
MimeType=application/x-csharp-code;text/x-csharp;
EOF
